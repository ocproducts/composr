...
function user_cache_row( $p_user_id, $p_trigger_errors = true ) {
	global $g_cache_user, $cms_sc_db_prefix;
	global $cms_updater_groups,$cms_developer_groups,$cms_manager_groups,$cms_admin_groups;
	...
	// HACKHACK: Rebuild row from Composr's DB
	$result = db_query_bound("SELECT * FROM ".$cms_sc_db_prefix."f_members WHERE id=" . db_param(), Array( $p_user_id ) );
	...
	$result = db_query_bound("SELECT gm_group_id FROM ".$cms_sc_db_prefix."f_group_members WHERE gm_validated=1 AND gm_member_id=" . db_param(), Array( $p_user_id ) );
	...
	$row = array (
		...
		'date_created'=>$cms_row['m_join_time'],
		'last_visit'=>$cms_row['m_last_visit_time'],
...
function user_get_id_by_name( $p_username ) {
	...
	$query = "SELECT id
				  FROM ".$cms_sc_db_prefix."f_members
