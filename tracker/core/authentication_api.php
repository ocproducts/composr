...
function auth_process_plain_password( $p_test_password, $p_real_password_bits ) {
	...
	switch ($password_compatibility_scheme)
	{
		...
		case 'vb3': // vBulletin (used on cmsortal.com a lot, for legacy reasons)
...
function auth_get_current_user_cookie( $p_login_anonymous=true ) {
	global $g_script_login_cookie, $g_cache_anonymous_user_cookie_string, $cms_sc_db_prefix, $cms_sc_session_cookie_name;
					...
					if (isset($_COOKIE[$cms_sc_session_cookie_name]))
					{
						...
						if( 1 == db_num_rows( $result ) ) {
							...
							$query = 'SELECT u.id,u.cookie_string
										  FROM '.$cms_sc_db_prefix.'f_members m LEFT JOIN ' . db_get_table( 'mantis_user_table' ) . ' u ON u.username=m.m_username
										  WHERE m.id<>1 AND m.id=' . strval($user);
							$result = db_query( $query );
						}
					}

					if((!isset($_COOKIE[$cms_sc_session_cookie_name])) || ( db_num_rows( $result ) == 0 )) {
