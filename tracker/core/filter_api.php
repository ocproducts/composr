	...
	for( $i = 0;$i < $t_count;$i++ ) {
		...
		if( !in_array( $t_sort_fields[$i], array_slice( $t_sort_fields, $i + 1 ) ) ) {
			...
			if( strpos( $c_sort, 'custom_' ) === 0 ) {
				...
				$c_cf_alias = str_replace( array(' ','(',')'), array('_','_','_'), $t_custom_field );
				...
				if( !in_array( $t_cf_select, $p_query_clauses['select'] ) ) {
					...
					$found = false;
					foreach ($p_query_clauses['join'] as $j)
					{
						if (strpos($j,$t_cf_table_alias)!==false) $found=true;
					}
					if (!$found)
				...
				$c_sort = $c_cf_alias;
				if (custom_field_type($t_custom_field_id) == 1) // Sort by integer
				{
					$c_sort = "CAST($c_cf_alias AS UNSIGNED)";
				}
...
function filter_get_bug_rows( &$p_page_number, &$p_per_page, &$p_page_count, &$p_bug_count, $p_custom_filter = null, $p_project_id = null, $p_user_id = null, $p_show_sticky = null ) {
	...
	if( !filter_field_is_any( $t_filter[FILTER_PROPERTY_MONITOR_USER_ID] ) ) {
		...
		foreach( $t_filter[FILTER_PROPERTY_MONITOR_USER_ID] as $t_filter_member ) {
			...
			if ($c_user_monitor == META_FILTER_NOT_MYSELF)
			{
				$t_where_clauses [] = "(NOT EXISTS (SELECT * FROM $t_bug_monitor_table $t_table_name WHERE $t_table_name.bug_id = $t_bug_table.id AND $t_table_name.user_id = $c_user_id))";
				continue;
			}
	...
	if( $p_expanded ) {
		...
		if( count( $t_filter[FILTER_PROPERTY_MONITOR_USER_ID] ) == 0 ) {
			...
			foreach( $t_filter[FILTER_PROPERTY_MONITOR_USER_ID] as $t_current ) {
				...
				else if ($t_current == META_FILTER_NOT_MYSELF) {
					$t_this_name = '[Not myself]';
				} else {
					$t_this_name = user_get_name( $t_current );
				}
				if( $t_first_flag != true ) {
...
function print_filter_relationship_type() {
	...
	echo '<input type="text" name="', FILTER_PROPERTY_RELATIONSHIP_BUG, '" size="5" maxlength="10" value="', string_attribute( $t_filter[FILTER_PROPERTY_RELATIONSHIP_BUG]), '" />';
}
