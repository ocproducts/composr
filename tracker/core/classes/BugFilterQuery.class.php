...
class BugFilterQuery extends DbQuery {
	...
	protected function helper_process_users_property( array $p_users_array ) {
		...
		foreach( $p_users_array as $t_user ) {
			if( filter_field_is_none( $t_user ) ) {
				...
				} else if( filter_field_is_not_myself( $c_user_id ) ) {
					$t_new_array[] = $this->user_id;
	...
	protected function build_prop_monitor_by() {
		...
		// Composr - Ability to find stuff you're not monitoring yourself
		if ($this->filter[FILTER_PROPERTY_MONITOR_USER_ID][0] == META_FILTER_NOT_MYSELF) {
			$t_where = "(NOT EXISTS (SELECT * FROM {bug_monitor} WHERE {bug_monitor}.bug_id = {bug}.id AND {bug_monitor}.user_id = " . strval( $this->user_id ) . "))";
			$this->add_where( $t_where );
			return;
		}
