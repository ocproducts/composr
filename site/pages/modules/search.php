...
class Module_search
{
    ...
    public function run()
    {
        ...
        cms_set_time_limit(15); // We really don't want to let it thrash the DB too long
    ...
    public function results($id, $author, $author_id, $cutoff, $sort, $direction, $only_titles, $search_under)
    {
        ...
        foreach ($_hooks as $hook => $ob) {
            ...
            if ((($id == '') || ($id == $hook)) && (($test == 1) || ((get_param_integer('all_defaults', 0) == 1) && ($info['default'])) || ($id == $hook))) {
                ...
                cms_set_time_limit(10); // Prevent errant search hooks (easily written!) taking down a server. Each call given 5 seconds (calling set_time_limit resets the timer).
