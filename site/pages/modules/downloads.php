...
class Module_downloads
{
    ...
    public function view_atoz_screen()
    {
        ...
        // Load up all data
        $cats = array();
        $rows = $GLOBALS['SITE_DB']->query('SELECT p.*,text_original FROM ' . get_table_prefix() . 'download_downloads p' . $privacy_join . ' WHERE ' . (addon_installed('unvalidated') ? 'validated=1 AND ' : '') . '(' . $sql_select . ')' . $privacy_where . ' ORDER BY text_original ASC', null, 0, false, true, array('name' => 'SHORT_TRANS'));
        foreach ($rows as $row) {
