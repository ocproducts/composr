...
class Module_warnings extends Standard_crud_module
{
    ...
    public function get_form_fields($new = true, $explanation = '', $is_warning = 0, $member_id = null)
    {
        ...
        if ($new) {
            ...
            if (addon_installed('securitylogging')) {
                if (has_actual_page_access(get_member(), 'admin_ip_ban')) {
                    $fields->attach(form_input_tick(do_lang_tempcode('WHETHER_BANNED_IP'), do_lang_tempcode('DESCRIPTION_WHETHER_BANNED_IP'), 'banned_ip', true));
                }
                ...
                if ($stopforumspam_api_key . $tornevall_api_username != '') {
                    require_lang('submitban');
                    $fields->attach(form_input_tick(do_lang_tempcode('SYNDICATE_TO_STOPFORUMSPAM'), do_lang_tempcode('DESCRIPTION_SYNDICATE_TO_STOPFORUMSPAM'), 'stopforumspam', true));
                }
            }

            if (has_privilege(get_member(), 'member_maintenance')) {
                $fields->attach(form_input_tick(do_lang_tempcode('BAN_MEMBER'), do_lang_tempcode('DESCRIPTION_BANNED_MEMBER'), 'banned_member', true));
            }
        ...
        if ($new) {
            $fields->attach(form_input_tick(do_lang_tempcode('ADD_PRIVATE_TOPIC'), do_lang_tempcode('DESCRIPTION_WARNING_TOPIC'), 'add_private_topic', false));
