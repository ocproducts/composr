		...
		CREATE TABLE cms10_banned_ip
		(
			i_ban_positive tinyint(1) NOT NULL,
			i_ban_until integer unsigned NOT NULL,
			ip varchar(40) NULL,
			i_descrip longtext NOT NULL,
			PRIMARY KEY (ip)
		) TYPE=InnoDB;

		CREATE TABLE cms10_usersubmitban_member
		(
		...
		CREATE INDEX `hackattack.member_id` ON cms10_hackattack(member_id);
		ALTER TABLE cms10_hackattack ADD FOREIGN KEY `hackattack.member_id` (member_id) REFERENCES cms10_f_members (id);

		CREATE INDEX `usersubmitban_member.the_member` ON cms10_usersubmitban_member(the_member);
		ALTER TABLE cms10_usersubmitban_member ADD FOREIGN KEY `usersubmitban_member.the_member` (the_member) REFERENCES cms10_f_members (id);
