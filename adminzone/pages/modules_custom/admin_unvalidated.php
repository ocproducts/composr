        ...
        foreach ($_hooks as $object) {
            ...
            if (!$content->is_empty()) {
                if (array_key_exists('uses_workflow', $info) && $info['uses_workflow']) {
                    ...
                    $post_url = build_url(array('page' => $info['view_module'], 'type' => $info['view_type'], 'validated' => 1/*, 'redirect' => get_self_url(true)*/), get_module_zone($info['view_module']), array(), false, true);
                } else {
                    // Content which isn't in a workflow is validated via its edit screen
                    $post_url = build_url(array('page' => $info['edit_module'], 'type' => $info['edit_type'], 'validated' => 1/*, 'redirect' => get_self_url(true)*/), get_module_zone($info['edit_module']), array(), false, true);
                }
