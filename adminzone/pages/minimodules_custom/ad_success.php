...
foreach ($advertiser_sessions as $session) {
    ...
    if (get_param_integer('track', 0) == 1) {
        ...
        foreach ($places as $place) {
            echo '<p>' . escape_html($place['the_page']) . ' at ' . get_timezoned_date($place['date_and_time'], false, false, false, true)/*TODO: fix in v11*/ . ' (from ' . escape_html(substr($place['referer'], 0, 200)) . ')</p>';
        }
    }
