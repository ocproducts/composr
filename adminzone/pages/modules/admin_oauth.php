...
class Module_admin_oauth
{
    ...
    {
        return array(
            'browse' => array('_OAUTH_TITLE', 'menu/adminzone/setup/config/config'), // TODO new icon needed
        );
    }
    ...
    public function browse()
    {
        ...
        foreach ($hooks as $service_name => $ob) {
            ...
            require_code('hooks/systems/config/' . $service_info['options']['client_id']);
            $ob = object_factory('Hook_config_' . $service_info['options']['client_id']);
            $info = $ob->get_details();
        ...
        return do_template('OAUTH_SCREEN', array('TITLE' => $this->title, 'SERVICES' => $services));
    }
