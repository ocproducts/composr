...
class images_test_set extends cms_test_case
{
    ...
    protected function checkRedPatch($file, $path, $image_resource, $x, $y, $image_dimensions, $color_tolerance, &$additional_information)
    {
        ...
        $additional_information = 'Expected pixel ' . strval($x) . 'x' . strval($y) . ' (or any pixels within ' . $tolerance . ') to have at least ' . (255 - $color_tolerance) . ' red, and more red than green and blue, but none of them were.';
        require_code('mime_types');
        echo '<br style="clear: both" />';
    ...
    public function testConvertImagePlus()
    {
        ...
        /*TODOforeach ($file_types as $extension) {
              // Basic box tests
              ...
              $this->assertTrue($this->runDimensionTestPlus(get_base_url() . '/_tests/assets/images/16x9.' . $extension, '240x', 240, null, 'box', 'height', false, $additional_information), 'convert_image_plus w/ 16x9 ' . $extension . '. Height algorithm where = both and only width specified. ' . $additional_information);
              $this->assertTrue($this->runDimensionTestPlus(get_base_url() . '/_tests/assets/images/16x9.' . $extension, 'x180', 180, null, 'box', 'height', false, $additional_information), 'convert_image_plus w/ 16x9 ' . $extension . '. Height algorithm where = both and only height specified. ' . $additional_information);
        ...
        $convert = convert_image_plus(get_base_url() . '/_tests/assets/images/pad_crop_both_32x18_16x9.png', '32x14', 'temp', null, null, 'pad_vert_crop_vert', 'both', '#000000', false);
        var_dump($convert);
