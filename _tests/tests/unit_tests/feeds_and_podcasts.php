...
class feeds_and_podcasts_test_set extends cms_test_case
{
    ...
    public function testXML()
    {
        ...
        $this->assertTrue(strpos($data, '</opml>') !== false);
        $parsed = new CMS_simple_xml_reader($data);

        $url = find_script('backend') . '?type=xslt-opml';
        $data = http_download_file($url, null, true, false, 'Composr', null, array(get_session_cookie() => get_session_id()));
        $this->assertTrue(strpos($data, '</xsl:stylesheet>') !== false);
        $parsed = new CMS_simple_xml_reader($data);
    ...
    public function testDayFilter()
    {
        ...
        $data = http_download_file($url, null, true, false, 'Composr', null, array(get_session_cookie() => get_session_id()));
        $this->assertTrue(strpos($data, '<item>') !== false);
    ...
    public function testPodcast()
    {
        ...
        $this->assertTrue(strpos($data, '<itunes:author>') !== false);
        $this->assertTrue(strpos($data, '<itunes:owner>') !== false);
        $this->assertTrue(strpos($data, '<itunes:name>') !== false);
        $this->assertTrue(strpos($data, '<itunes:email>') !== false);
        $this->assertTrue(strpos($data, '<itunes:image') !== false);
        $this->assertTrue(strpos($data, '<itunes:category') !== false);
        $this->assertTrue(strpos($data, '<itunes:keywords>') !== false);
        $this->assertTrue(strpos($data, '<itunes:summary>') !== false);
        $this->assertTrue(strpos($data, '<itunes:author>') !== false);
        $this->assertTrue(strpos($data, '<itunes:image') !== false);
        $this->assertTrue(strpos($data, '<itunes:duration>') !== false);
    }
