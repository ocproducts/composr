...
function find_request_metadata($include_referer = true)
{
    ...
    if (!is_guest()) {
        $data[do_lang('cns:PROFILE')] = $GLOBALS['FORUM_DRIVER']->member_profile_url(get_member(), true, false);
    }
    ...
    if (function_exists('json_decode')) { // TODO: Fix on v11
        $_json = http_download_file('http://freegeoip.net/json/' . get_ip_address(), null, false);
    ...
    if (addon_installed('stats')) {
        ...
        foreach ($pages as $myrow) {
            ...
            $h[do_lang('DATE_TIME')] = get_timezoned_date(tz_time($myrow['date_and_time'], get_server_timezone()), true, false, false, true);
