...
function delete_pending_orders_for_current_user($keep_order_id = null, $purchase_through = null)
{
    ...
    $orders = $GLOBALS['SITE_DB']->query_select('shopping_orders', array('id', 'notes'), $where, $extra);
    foreach ($orders as $order) {
        if ($order['id'] !== $keep_order_id) {
            if ($order['notes'] == '') {
                $GLOBALS['SITE_DB']->query_delete('shopping_order_details', array('order_id' => $order['id']));
                $GLOBALS['SITE_DB']->query_delete('shopping_orders', array('id' => $order['id']), '', 1);
            } else {
                ...
                $GLOBALS['SITE_DB']->query_update('shopping_orders', array('order_status' => 'ORDER_STATUS_cancelled'), array('id' => $order['id']), '', 1);
            }
        }
    }
}
