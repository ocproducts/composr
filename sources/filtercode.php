...
function _default_conv_func($db, $info, $catalogue_name, &$extra_join, &$extra_select, $filter_key, $filter_val, $db_fields, $table_join_code)
{
    ...
    if (($filter_key == 'compound_rating') || (preg_match('#^compound_rating\_\_(.+)#', $filter_key, $matches) != 0)) {
        ...
        $clause = '(SELECT SUM(rating-1) FROM ' . $db->get_table_prefix() . 'rating rat WHERE ' . db_string_equal_to('rat.rating_for_type', $matches[1]) . ' AND rat.rating_for_id=' . $table_join_code . '.' . $info['id_field'] . ')';
    ...
    if (($filter_key == 'average_rating') || (preg_match('#^average_rating\_\_(.+)#', $filter_key, $matches) != 0)) {
        ...
        $clause = '(SELECT AVG(rating)/2 FROM ' . $db->get_table_prefix() . 'rating rat WHERE ' . db_string_equal_to('rat.rating_for_type', $matches[1]) . ' AND rat.rating_for_id=' . $table_join_code . '.' . $info['id_field'] . ')';
    ...
    if ($filter_key == 'meta_description') {
        ...
        $join = ' LEFT JOIN ' . $db->get_table_prefix() . 'seo_meta sm ON sm.meta_for_id=' . $table_join_code . '.' . $info['id_field'] . ' AND ' . db_string_equal_to('sm.meta_for_type', $seo_type_code);
    ...
    if ($filter_key == 'meta_keywords') {
        ...
        if (multi_lang_content()) {
            $clause = '(SELECT GROUP_CONCAT(text_original) FROM ' . $db->get_table_prefix() . 'seo_meta_keywords kw JOIN ' . $db->get_table_prefix() . 'translate kwt ON kwt.id=kw.meta_keyword WHERE kw.meta_for_id=' . $table_join_code . '.' . $info['id_field'] . ' AND ' . db_string_equal_to('kw.meta_for_type', $seo_type_code) . ')';
        } else {
            $clause = '(SELECT GROUP_CONCAT(meta_keyword) FROM ' . $db->get_table_prefix() . 'seo_meta_keywords kw WHERE kw.meta_for_id=' . $table_join_code . '.' . $info['id_field'] . ' AND ' . db_string_equal_to('kw.meta_for_type', $seo_type_code) . ')';
        }
