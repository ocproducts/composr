    ...
    do {
        ...
        foreach ($events as $event) {
            ...
            if (!is_guest($event['e_submitter'])) {
                $username = $GLOBALS['FORUM_DRIVER']->get_username($event['e_submitter'], true, USERNAME_DEFAULT_NULL);
                if ($username !== null) {
                    echo "ORGANIZER;CN=" . ical_escape($username) . ";DIR=" . ical_escape($GLOBALS['FORUM_DRIVER']->member_profile_url($event['e_submitter'], false));
            ...
            do {
                ...
                if (is_array($_comments)) {
                    foreach ($_comments as $comment) {
                        ...
                        echo "COMMENT:" . ical_escape(strip_comcode(is_object($comment['message']) ? $comment['message'] : $comment['message']) . ' - ' . $username . ' (' . get_timezoned_date_time($comment['date']) . ')') . "\r\n";
                    }
                } else {
                    break;
                }
            ...
            foreach ($attendees as $attendee) {
                if ($attendee['n_member_id'] != get_member()) {
                    if (!is_guest($event['n_member_id'])) {
                        $username = $GLOBALS['FORUM_DRIVER']->get_username($attendee['n_member_id'], true, USERNAME_DEFAULT_NULL);
                        if ($username !== null) {
                            echo "ATTENDEE;CN=" . ical_escape($username) . ";DIR=" . ical_escape($GLOBALS['FORUM_DRIVER']->member_profile_url($attendee['n_member_id'], false));
                        }
                    }
