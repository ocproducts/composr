...
class Hook_rss_cns_unread_topics
{
    ...
    public function run($_filters, $cutoff, $prefix, $date_string, $max)
    {
        ...
        $query .= ' FROM ' . $GLOBALS['FORUM_DB']->get_table_prefix() . 'f_topics top LEFT JOIN ' . $GLOBALS['FORUM_DB']->get_table_prefix() . 'f_read_logs l ON t.id=l.l_topic_id AND l.l_member_id=' . strval((integer)get_member());
        if (!multi_lang_content()) {
