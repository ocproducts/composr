 ...
 * Hook class.
 */
class Hook_commandr_fs_download_licences extends Resource_fs_base
{
    ...
    {
        $_ret = $GLOBALS['SITE_DB']->query_select('download_licences', array('id'), array('l_title' => $label), 'ORDER BY id');
        $ret = array();
        foreach ($_ret as $r) {
            $ret[] = strval($r['id']);
        }
    ...
    {
        $query = 'SELECT MAX(date_and_time) FROM ' . get_table_prefix() . 'actionlogs WHERE ' . db_string_equal_to('param_a', strval($row['id'])) . ' AND  (' . db_string_equal_to('the_type', 'ADD_DOWNLOAD_LICENCE') . ' OR ' . db_string_equal_to('the_type', 'EDIT_DOWNLOAD_LICENCE') . ')';
