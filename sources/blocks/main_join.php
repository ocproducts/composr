...
class Block_main_join
{
    ...
    public function run($map)
    {
        ...
        if (!is_guest()) {
            return do_template('BLOCK_MAIN_JOIN_DONE', array('_GUID' => $guid, 'MESSAGE' => null, 'READY' => true));
        }
        ...
        if ($username != '') {
            ...
            if ($email_address != '') {
                ...
                if (file_exists(get_custom_file_base() . '/' . $map['path'])) {
                    ...
                    $subject = empty($map['subject']) ? do_lang('WELCOME') : $map['subject'];
                    ...
                    require_code('mail');
                    mail_wrap($subject, $body, array($email_address), array_key_exists('to', $map) ? $map['to'] : '', '', '', 3, null, false, null, true);
                }
            }
            ...
            return do_template('BLOCK_MAIN_JOIN_DONE', array('_GUID' => $guid, 'MESSAGE' => $message, 'READY' => $ready));
        } else {
            $post_url = get_self_url();

            list($javascript, $form) = cns_join_form($post_url, $captcha_if_enabled, false, true, $adjusted_config_options);

            return do_template('BLOCK_MAIN_JOIN', array('_GUID' => $guid, 'FORM' => $form, 'JAVASCRIPT' => $javascript));
        }
    }
}
