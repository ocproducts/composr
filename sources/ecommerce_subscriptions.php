...
function find_member_subscriptions($member_id, $usergroup_subscriptions_only = false)
{
    ...
    if ((get_forum_type() == 'cns') && (addon_installed('ecommerce'))) {
        ...
        $_subscriptions_non_recurring = $GLOBALS['FORUM_DB']->query_select('f_group_member_timeouts', array('*'), array('member_id' => $member_id));
...
function prepare_templated_subscription($subscription)
{
    ...
    return array(
        ...
        'CANCEL_BUTTON' => (($subscription['state'] == 'active') && ($subscription['auto_fund_key'] != '')) ? make_cancel_button($subscription['auto_fund_key'], $subscription['payment_gateway']) : new Tempcode(),
    );
}
