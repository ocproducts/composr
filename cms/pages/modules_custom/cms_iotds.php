...
class Module_cms_iotds extends Standard_crud_module
{
    ...
    public function get_form_fields($id = null, $url = '', $thumb_url = '', $title = '', $caption = '', $current = true, $allow_rating = 1, $allow_comments = 1, $allow_trackbacks = 1, $notes = '')
    {
        ...
        if (has_privilege(get_member(), 'choose_iotd')) {
            ...
            $fields->attach(form_input_tick(do_lang_tempcode('IMMEDIATE_USE'), do_lang_tempcode(($id == '') ? 'DESCRIPTION_IMMEDIATE_USE_ADD' : 'DESCRIPTION_IMMEDIATE_USE'), 'validated', $current));
        }
    ...
    public function edit_actualisation($_id)
    {
        ...
        edit_iotd($id, $title, post_param_string('caption'), $thumb_url, $url, $allow_rating, $allow_comments, $allow_trackbacks, $notes, $meta_data['edit_time'], $meta_data['add_time'], $meta_data['views'], $meta_data['submitter'], true);
