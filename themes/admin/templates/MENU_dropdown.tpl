...
{+START,IF_NON_EMPTY,{CONTENT}}
	<nav class="menu_type__dropdown">
		<ul onmouseout="return deset_active_menu()" class="nl" id="r_{MENU|*}_d">
			...
			<li class="non_current last toplevel"{+START,IF,{$GET,HAS_CHILDREN}}{+START,IF,{$DESKTOP}} onmousemove="window.menu_hold_time=3000; if (!this.timer) this.timer=window.setTimeout(function() { var ret=pop_up_menu('{MENU|;*}_dexpand_{$GET;*,RAND}','below','{MENU|;*}_d',event,true); try { document.getElementById('search_content').focus(); } catch (e) {} return ret; } , 200);" onmouseout="if (this.timer) { window.clearTimeout(this.timer); this.timer=null; }"{+END}{+END}>
				<a href="{$TUTORIAL_URL*,tutorials}" onclick="cancel_bubbling(event);{+START,IF,{$GET,HAS_CHILDREN}} deset_active_menu();{+END}" class="toplevel_link last"{+START,IF,{$HAS_ACTUAL_PAGE_ACCESS,admin,adminzone}} onfocus="return pop_up_menu('{MENU|;*}_dexpand_{$GET;*,RAND}','below','{MENU|;*}_d',event,true);"{+END} title="{!menus:MM_TOOLTIP_DOCS}"><img width="32" height="32" alt="" src="{$IMG*,icons/32x32/menu/adminzone/help}" srcset="{$IMG*,icons/64x64/menu/adminzone/help} 2x" /> <span>{!HELP}</span></a>
