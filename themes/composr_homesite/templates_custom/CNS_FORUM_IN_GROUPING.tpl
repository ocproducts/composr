<tr>
		...
		{+START,IF,{$MOBILE}}
			<div role="note">
				<ul class="horizontal_meta_details associated_details">
					<li><img width="15" height="15" title="{LANG_NEW_POST_OR_NOT*}" alt="{LANG_NEW_POST_OR_NOT*}" src="{$IMG*,cns_general/{NEW_POST_OR_NOT*}}" /></li>
	...
	<td class="cns_forum_latest cns_column6">
		{+START,IF,{$NEQ,{FORUM_NAME},Feature requests and bug tracking}}
			{LATEST}
		{+END}

		{+START,IF,{$EQ,{FORUM_NAME},Feature requests and bug tracking}}
			<strong>{$MANTIS_WEEK_ACTIVITY*}</strong> modified in last 7 days<br />
			<strong>{$MANTIS_SPONSOR_WEEK_ACTIVITY*}</strong> sponsorships added
		{+END}
	</td>
</tr>
