...
{+START,IF_NON_EMPTY,{URL}}<a{+START,IF,{$NOT,{$INLINE_STATS}}} onclick="return ga_track(this,'{!BANNER;*}','{DEST;*}');"{+END} rel="nofollow external" title="{!BANNER}{+START,IF_NON_EMPTY,{CAPTION}}: {CAPTION*}{+END} {!LINK_NEW_WINDOW}" class="link_exempt"{+START,IF_NON_PASSED_OR_FALSE,LOCAL} target="_blank"{+END} href="{$FIND_SCRIPT*,banner}?source={SOURCE&*}&amp;dest={DEST&*}&amp;type=click{$KEEP*,0,1}">{+END}<img width="{WIDTH*}" height="{HEIGHT*}" alt="{!BANNER}{+START,IF_NON_EMPTY,{CAPTION}}: {CAPTION*}{+END}" style="border: 0" title="{CAPTION*}" src="{$ENSURE_PROTOCOL_SUITABILITY*,{IMG}}" />{+START,IF_NON_EMPTY,{URL}}</a>{+END}
