			...
			{+START,IF_NON_EMPTY,{SLIDESHOW_URL}}
				{+START,IF,{$DESKTOP}}
					{+START,IF,{$NOT,{SLIDESHOW}}}
						<a class="button_screen buttons--slideshow inline-desktop" rel="nofollow"{+START,IF,{$NOT,{$MOBILE}}} target="_blank" title="{!SLIDESHOW} {!LINK_NEW_WINDOW}"{+END} href="{SLIDESHOW_URL*}"><span>{!_SLIDESHOW}</span></a>
					{+END}
				{+END}
			{+END}
