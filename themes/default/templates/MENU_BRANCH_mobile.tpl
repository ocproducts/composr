...
<li class="{$?,{CURRENT},current,non_current} {$?,{$IS_EMPTY,{$GET,img}},has_no_img,has_img}">
	<a class="{+START,IF_EMPTY,{URL}}non_link {+END}{+START,IF,{$GET,HAS_CHILDREN}} drawer js-click-toggle-sub-menu{+END}" {+START,IF,{$GET,HAS_CHILDREN}} data-vw-sub-menu-id="{MENU|*}_pexpand_{$GET*,RAND}"{+END} href="{$?*,{$IS_EMPTY,{URL}},#!,{URL}}">{+START,IF_NON_EMPTY,{$GET,img}}<img alt="" src="{$GET*,img}" srcset="{$GET*,img_2x} 2x" /> {+END}{CAPTION}</a>
