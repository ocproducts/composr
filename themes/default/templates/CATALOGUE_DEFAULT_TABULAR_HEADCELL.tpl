...
<th>
	...
	<span class="table_header_sorting">
		...
		{+START,IF_NON_EMPTY,{SORT_URL_ASC}}<a target="_self" href="{SORT_URL_ASC*}"><img src="{$IMG*,results/{$?,{SORT_ASC_SELECTED},sortablefield_asc,sortablefield_asc_nonselected}}" title="{!SORT_BY} {FIELD*}, {!ASCENDING}" alt="{!SORT_BY} {FIELD*}, {!ASCENDING}" /></a>{+END}
		{+END}
		{+START,IF,{$NEQ,{$PAGE},search}}
