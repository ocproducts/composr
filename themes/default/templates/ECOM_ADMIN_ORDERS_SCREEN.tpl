...
<form title="{!SEARCH}" target="_self" method="get" action="{SEARCH_URL*}" onsubmit="try { window.scrollTo(0,0); } catch(e) {}" autocomplete="off">
	<div>
		...
		<label for="order_filter">
			...
			<input maxlength="255" type="text" id="order_filter" name="search" value="{SEARCH_VAL*}" /><input data-disable-on-click="{}" class="button_micro buttons__search" type="submit" value="{!SEARCH}" /> ({!SEARCH_ORDERS})
		</label>
	</div>
</form>
