...
<div class="box cns-notification" data-tpl="cnsNotification" data-tpl-params="{+START,PARAMS_JSON,IGNORE_URL_2}{_*}{+END}">
	<div class="box-inner" data-toggleable-tray="{}">
		<p class="cns-notification-intro-line js-tray-onclick-toggle-tray">
			<a class="toggleable-tray-button js-tray-onclick-toggle-tray" href="#!"><img alt="{!EXPAND}: {TYPE*}" title="{!EXPAND}" width="24" height="24" src="{$IMG*,1x/trays/expand2}" /></a>
		...
		<div class="toggleable-tray js-tray-content" style="display: none" aria-expanded="false">
			...
			<ul class="horizontal-links associated-links-block-group force-margin">
				<li><span><a href="{TOPIC_URL*}" title="{!VIEW}: {!FORUM_POST} #{ID*}">{!VIEW}</a></span>{+START,IF,{$NEQ,{_ADDITIONAL_POSTS},0}} <img data-cms-rich-tooltip="{}" title="{!cns:ADDITIONAL_PT_POSTS,{ADDITIONAL_POSTS}}" alt="{!HELP}" width="24" height="24" src="{$IMG*,icons/32x32/help}" class="top-vertical-alignment help-icon" />{+END}</li>
