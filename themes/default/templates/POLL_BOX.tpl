...
<section class="box box---poll-box" data-view="PollBox"><div class="box-inner">
	...
	<ul class="horizontal-links associated-links-block-group">
		...
		{+START,IF_NON_EMPTY,{RESULT_URL}}<li><form title="{!POLL_RESULTS}" target="_self" class="inline" action="{VOTE_URL*}" method="post" autocomplete="off">{$INSERT_SPAMMER_BLACKHOLE}<button data-click-pd="1" class="button-hyperlink js-click-confirm-forfeit" type="submit">{!POLL_RESULTS}</button></form></li>{+END}
