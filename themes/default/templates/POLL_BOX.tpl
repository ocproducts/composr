<section class="box box___poll_box"><div class="box_inner">
	...
	<ul class="horizontal_links associated_links_block_group">
		...
		{+START,IF_NON_EMPTY,{RESULT_URL}}<li><form title="{!POLL_RESULTS}" target="_self" class="inline" action="{VOTE_URL*}" method="post">{$INSERT_SPAMMER_BLACKHOLE}<input onclick="var form=this.form; window.fauxmodal_confirm('{!VOTE_FORFEIGHT}'\,function(answer) \{ if (answer) guarded_form_submit(form); \}); return false;" class="button_hyperlink" type="submit" value="{!POLL_RESULTS}" /></form></li>{+END}
