...
<div class="search-option" data-tpl="cnsMemberDirectoryScreenFilter">
	...
	{+START,IF_NON_EMPTY,{$CPF_LIST,{NAME}}}
		<select class="wide_field search-option-value" id="filter_{BLOCK_ID*}_{NAME|*}" name="filter_{BLOCK_ID*}_{NAME|*}">
			<option value="">---</option>
	...
	{+START,IF_EMPTY,{$CPF_LIST,{NAME}}}
		...
		<input {+START,IF,{$MOBILE}} autocorrect="off"{+END} autocomplete="off" class="wide_field search-option-value {+START,IF,{$EQ,{LABEL},{!USERNAME}}}js-keyup-input-filter-update-ajax-member-list{+END}" type="text" value="{$_GET*,filter_{BLOCK_ID}_{NAME|}}" id="filter_{BLOCK_ID*}_{NAME|*}" name="filter_{BLOCK_ID*}_{NAME|*}" />
	{+END}
</div>
