	...
	<td id="cell_mark_{ID*}" class="cns_topic_marker_cell">
		<form class="webstandards_checker_off inline" title="{!MARKER} #{ID*}" method="post" action="index.php" id="form_mark_{ID*}" autocomplete="off">
			...
			<div class="inline">
				...
				<input value="1" type="checkbox" id="mark_{ID*}" name="mark_{ID*}" onclick="change_class(this,this.parentNode.parentNode.parentNode.parentNode,'cns_on','cns_off')" {+START,IF,{$NOT,{$IS_GUEST}}} title="{!MARKER} #{ID*}"{+END} />
			</div>
		</form>
	</td>
{+END}
