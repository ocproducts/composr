		...
		{+START,SET,sort_button}
			{+START,IF,{$OR,{$GET,show_sort_button},{$NOT,{$JS_ON}}}}
				{+START,IF_NON_PASSED,FILTER}
					<input data-disable-after-click="1" class="button_micro buttons__sort" type="submit" title="{!SORT_BY}{+START,IF_NON_EMPTY,{$GET,TEXT_ID}}: {$GET*,TEXT_ID}{+END}" value="{!SORT_BY}" />
				{+END}
			{+END}
			{+START,IF_PASSED,FILTER}
				<input data-disable-after-click="1" class="button_micro buttons__filter" type="submit" title="{!PROCEED}{+START,IF_NON_EMPTY,{$GET,TEXT_ID}}: {$GET*,TEXT_ID}{+END}" value="{!PROCEED}" />
			{+END}
		{+END}
