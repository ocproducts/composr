		...
		{+START,SET,sort_button}
			{+START,IF,{$OR,{$GET,show_sort_button},{$NOT,{$JS_ON}}}}
				{+START,IF_NON_PASSED,FILTER}
					<input onclick="disable_button_just_clicked(this);" class="button_micro buttons__sort" type="submit" title="{!SORT_BY}{+START,IF_NON_EMPTY,{$GET,TEXT_ID}}: {$GET*,TEXT_ID}{+END}" value="{!SORT_BY}" />
				{+END}
			{+END}
