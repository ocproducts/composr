	...
	$(function() {
		...
		{+START,IF,{$AND,{ALLOW_INFINITE_SCROLL},{$NEQ,{$_GET,keep_infinite_scroll},0}}}
			{+START,IF,{$CONFIG_OPTION,infinite_scrolling}}
				...
				window.addEventListener('scroll',infinite_scrolling_{$GET%,wrapper_id});
				window.addEventListener('touchmove',infinite_scrolling_{$GET%,wrapper_id});
				window.addEventListener('keydown',infinite_scrolling_block);
				window.addEventListener('mousedown',infinite_scrolling_block_hold);
				window.addEventListener('mousemove',function() { infinite_scrolling_block_unhold(infinite_scrolling_{$GET%,wrapper_id}); }); // mouseup/mousemove does not work on scrollbar, so best is to notice when mouse moves again (we know we're off-scrollbar then)
