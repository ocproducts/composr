...
{+START,IF,{$NOT,{$IS_GUEST}}}{+START,IF,{$NOTIFICATIONS_AVAILABLE,{$GET,NOTIFICATIONS_TYPE}}}
	...
	{$INC,notification_id}
	<form id="nenable_{$GET*,notification_id}" title="{!notifications:NOTIFICATIONS}"{+START,IF,{$NOTIFICATIONS_ENABLED,{NOTIFICATIONS_ID},{$GET,NOTIFICATIONS_TYPE}}} style="display: none" aria-hidden="true"{+END} onsubmit="set_display_with_aria(this,'none'); set_display_with_aria(document.getElementById('ndisable_{$GET;*,notification_id}'),'inline');" data-open-as-overlay="{}" class="inline" rel="enable-notifications" method="post" action="{$PAGE_LINK*,{$GET,NOTIFICATIONS_PAGE_LINK}:redirect={$SELF_URL&*,1,0,0,wide_high=<null>}}" autocomplete="off">{$INSERT_SPAMMER_BLACKHOLE,1}<input type="submit" class="buttons__enable_notifications {$GET*,button_type}" value="{$GET,button_label_enable}" /></form>
	<form id="ndisable_{$GET*,notification_id}" title="{!notifications:NOTIFICATIONS}"{+START,IF,{$NOT,{$NOTIFICATIONS_ENABLED,{NOTIFICATIONS_ID},{$GET,NOTIFICATIONS_TYPE}}}} style="display: none" aria-hidden="true"{+END} onsubmit="set_display_with_aria(this,'none'); set_display_with_aria(document.getElementById('nenable_{$GET;*,notification_id}'),'inline');" data-open-as-overlay="{}" class="inline" rel="disable-notifications" method="post" action="{$PAGE_LINK*,{$GET,NOTIFICATIONS_PAGE_LINK}:redirect={$SELF_URL&*,1,0,0,wide_high=<null>}}" autocomplete="off">{$INSERT_SPAMMER_BLACKHOLE,1}<input type="submit" class="buttons__disable_notifications {$GET*,button_type}" value="{$GET,button_label_disable}" /></form>
