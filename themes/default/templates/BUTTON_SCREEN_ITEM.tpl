{+START,IF,{$OR,{$NEQ,{$PAGE},members},{$NEQ,{IMG},buttons__new_quote}}}
	...
	{+START,IF,{$NOT,{IMMEDIATE}}}
		<a data-tpl="buttonScreenItem" class="{IMG*} button_screen_item" {+START,IF_PASSED,TARGET} target="{TARGET*}"{+END}{+START,IF_PASSED,REL} rel="{REL*}"{+END} {+START,IF_PASSED,EXTRA_ATRRS}{EXTRA_ATTRS}{+END} {+START,IF_PASSED,JAVASCRIPT} data-click-eval="{JAVASCRIPT*}" {+END} href="{URL*}">
			<span>{TITLE*}</span>
		</a>
	{+END}
