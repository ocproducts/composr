...
{+START,IF_NON_EMPTY,{URL}}
	<p class="purchase_button">
		<input id="proceed_button" onclick="if (typeof this.form=='undefined') var form=window.form_submitting; else var form=this.form; return do_form_submit(form,event);" class="button_screen {ICON*}" accesskey="u"{+START,IF,{$JS_ON}} type="button"{+END}{+START,IF,{$NOT,{$JS_ON}}} type="submit"{+END} value="{SUBMIT_NAME*}" />
	</p>
</form>
{+END}
