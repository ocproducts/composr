				...
				{+START,IF,{$DESKTOP}}
					{+START,IF_NON_EMPTY,{MODERATOR_ACTIONS}}{+START,IF_NON_EMPTY,{MODERATOR_ACTIONS}}{+START,IF,{$NOT,{$_GET,overlay}}}
						<th class="cns-forum-box-right">
							<a href="#!" class="js-click-mark-all-topics"><img width="14" height="14" src="{$IMG*,icons/cns_topic_modifiers/unvalidated}" alt="{!TOGGLE_SELECTION}" title="{!TOGGLE_SELECTION}" /></a>
						</th>
					{+END}{+END}{+END}
				{+END}
			</tr>
				...
				<form title="{!TOPIC_ACTIONS}" action="{$URL_FOR_GET_FORM*,{ACTION_URL}}" method="get" class="inline" autocomplete="off">
					...
					<div class="inline">
						<select class="dropdown-actions" name="type" id="fma-type">
							...
							{MODERATOR_ACTIONS}
						</select><button class="button-micro buttons--proceed js-click-btn-add-form-marked-posts" type="submit">{!PROCEED}</button>
					</div>
				</form>
