{+START,SET,branch}
	...
	{+START,IF,{TOP_LEVEL}}
		<li class="dropdown-menu-item {$?,{CURRENT},current,non-current}{+START,IF,{$GET,HAS_CHILDREN}} has-children{+END}{+START,IF,{$AND,{$NOT,{$GET,HAS_CHILDREN}},{LAST}}} last{+END}{+START,IF,{FIRST}} first{+END} toplevel{+START,IF,{$GET,HAS_CHILDREN}} js-mousemove-timer-pop-up-menu js-mouseout-clear-pop-up-timer{+END}" data-vw-rand="{$GET*,RAND}">
			<a{+START,INCLUDE,MENU_LINK_PROPERTIES}{+END} class="dropdown-menu-item-a{+START,IF_EMPTY,{URL}} non-link{+END} toplevel-link{+START,IF,{LAST}} last{+END}{+START,IF,{FIRST}} first{+END}{+START,IF,{$GET,HAS_CHILDREN}} js-focus-pop-up-menu js-click-unset-active-menu js-click-toggle-sub-menu{+END}"{+START,IF,{$GET,HAS_CHILDREN}} data-vw-sub-menu-id="{MENU|*}-dexpand-{$GET*,RAND}"{+END} data-vw-rand="{$GET*,RAND}">
