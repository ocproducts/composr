<th>
	...
	<span class="table_header_sorting">
		...
		{+END}
		{+START,IF,{$NEQ,{$PAGE},search}}
		{+START,IF_NON_EMPTY,{SORT_URL_DESC}}<a target="_self" href="{SORT_URL_DESC*}"><img src="{$IMG*,results/{$?,{SORT_DESC_SELECTED},sortablefield_desc,sortablefield_desc_nonselected}}" title="{!SORT_BY} {FIELD*}, {!LARGEST_FIRST}" alt="{!SORT_BY} {FIELD*}, {!DESCENDING}" /></a>{+END}
		{+END}
	</span>
</th>
