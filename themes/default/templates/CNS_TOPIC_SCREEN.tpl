	...
	<div class="cns-topic-{THREADED*}">
		...
		{+START,IF,{$NOT,{THREADED}}}
			{+START,IF_NON_EMPTY,{POSTS}}
				<div class="wide_table_wrap"><div class="wide_table cns-topic autosized_table">
					{POSTS}
				</div></div>
			{+END}
		{+END}
		...
		{+START,IF,{$OR,{$IS_NON_EMPTY,{MODERATOR_ACTIONS}},{$IS_NON_EMPTY,{MARKED_POST_ACTIONS}},{THREADED}}}
			<div class="box cns-topic-control-functions{+START,IF,{$NOR,{$IS_NON_EMPTY,{MARKED_POST_ACTIONS}},{THREADED}}} block_desktop{+END}"><div class="box_inner">
					...
					<form title="{!TOPIC_ACTIONS}" action="{$URL_FOR_GET_FORM*,{ACTION_URL}}" method="get" class="inline horiz-field-sep-rightward" autocomplete="off">
						...
						<div class="inline">
							...
							<select class="dropdown-actions" id="tma_type" name="type">
								...
								{MODERATOR_ACTIONS}
							</select><input class="button_micro buttons--proceed js-click-require-tma-type-selection" type="submit" value="{!PROCEED}" />
						</div>
					</form>
				{+END}
						...
						<form title="{!MARKED_POST_ACTIONS}" action="{$URL_FOR_GET_FORM*,{ACTION_URL}}" method="get" class="inline horiz-field-sep-rightward block_desktop js-form-marked-post-actions" autocomplete="off">
							...
							<div class="inline">
								...
								<select id="mpa_type" name="type">
									...
									{+END}
									{MARKED_POST_ACTIONS}
								</select><input class="button_micro buttons--proceed js-click-check-marked-form-and-submit" type="submit" value="{!PROCEED}" />
							</div>
						</form>
					{+END}
				{+END}
				...
				{+START,IF,{THREADED}}
					<form class="inline" action="{$SELF_URL*}" method="post" autocomplete="off">
						...
						<div class="inline">
							...
							<input type="submit" value="{!SORT}" class="button_micro buttons--sort" />
						</div>
					</form>
				{+END}
			</div></div>
		...
		{+START,SET,double_post_message}
			{+START,IF_EMPTY,{QUICK_REPLY}}{+START,IF,{$EQ,{LAST_POSTER},{$MEMBER}}}{+START,IF,{$NOT,{$IS_GUEST}}}{+START,IF,{$NOT,{MAY_DOUBLE_POST}}}
				<div class="box box---members-viewing"><div class="box_inner">
					{!NO_DOUBLE_POST}
				</div></div>
			{+END}{+END}{+END}{+END}
		{+END}
