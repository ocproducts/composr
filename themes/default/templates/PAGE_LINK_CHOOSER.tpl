...
{+END}
	<div>
		...
		<input onchange="{+START,IF_PASSED_AND_TRUE,GET_TITLE_TOO}if (typeof this.selected_title=='undefined') { this.value=''; /*was autocomplete, unwanted*/ return; } this.value+=' '+this.selected_title;{+END}{+START,IF_NON_PASSED_OR_FALSE,AS_FIELD}window.returnValue=this.value; if (typeof window.faux_close!='undefined') window.faux_close(); else window.close();{+END}" style="display: none" type="text" id="{NAME*}" name="{NAME*}" value="{VALUE*}" />
