...
{+START,IF_NON_EMPTY,{RESET_URL}}
	<form title="{!RESET_BANNER_STATS}" action="{RESET_URL*}" method="post" autocomplete="off">
		...
		<button class="btn btn-primary btn-scr admin--delete3" type="submit" data-cms-confirm-click="{!WARNING_RESET_BANNER_STATS*}">{+START,INCLUDE,ICON}NAME=admin/delete3{+END} {!RESET_BANNER_STATS}</button>
	</form>
{+END}
