...
{+START,IF,{$OR,{LIKES},{$IS_EMPTY,{$TRIM,{RATING_FORM}}}}}
	...
	{+START,WHILE,{$LT,{$GET,rating_loop},{$GET,rating_stars}}}<img width="14" height="14" src="{$IMG*,icons/feedback/rating}" {$?,{$EQ,{$GET,rating_loop},0},alt="{!HAS_RATING,{$GET,rating_stars}}" title="{!HAS_RATING,{$GET,rating_stars}}",alt=""} />{$INC,rating_loop}{+END}
