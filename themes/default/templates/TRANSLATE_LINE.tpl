<tr>
	...
	<td class="translate_line_second" onmouseover="document.getElementById('trans_{NAME;*}').disabled=false;">
		...
		<div class="constrain_field">
			<textarea {+START,IF,{$EQ,{OLD},{CURRENT}}} disabled="disabled"{+END} {+START,IF_PASSED,TRANSLATE_AUTO} onclick="if (this.value==='') this.value='{TRANSLATE_AUTO;^*}';"{+END} class="wide_field translate_field" cols="60" rows="{+START,IF,{$EQ,{CURRENT},}}{$ADD,{$DIV,{$LENGTH,{OLD}},80},1}{+END}{+START,IF,{$NEQ,{CURRENT},}}{$ADD,{$DIV,{$LENGTH,{CURRENT}},80},1}{+END}" id="trans_{NAME*}" name="{NAME*}">{CURRENT*}</textarea>
		</div>
	</td>
