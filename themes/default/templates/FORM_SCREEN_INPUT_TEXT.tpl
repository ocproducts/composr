...
<div id="container_for_{NAME*}" data-tpl="formScreenInputText" data-tpl-params="{+START,PARAMS_JSON,REQUIRED,NAME}{_*}{+END}">
	<textarea data-textarea-auto-height="" tabindex="{TABINDEX*}" class="input-text{REQUIRED*} wide-field{+START,IF,{SCROLLS}} textarea_scroll{+END}" cols="70" rows="{+START,IF_PASSED,ROWS}{ROWS*}{+END}{+START,IF_NON_PASSED,ROWS}7{+END}" id="{NAME*}" name="{NAME*}"{+START,IF_PASSED,MAXLENGTH} maxlength="{MAXLENGTH*}"{+END}>{DEFAULT*}</textarea>
