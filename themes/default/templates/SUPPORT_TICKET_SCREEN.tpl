					...
					<li>
						...
						{+START,IF,{$HAS_PRIVILEGE,support_operator}}
							<form title="{!_ASSIGNED_TO}" action="{$PAGE_LINK*,_SEARCH:tickets:unassign:ticket_id={ID}:member_id={_loop_key}}" method="post" class="inline vertical-alignment" autocomplete="off">
								...
								<button class="btn btn-primary btn-sm admin--delete3" type="submit">{+START,INCLUDE,ICON}NAME=admin/delete3{+END} {!REMOVE}</button>
							</form>
						{+END}
					</li>
				{+END}
			...
			{+START,IF,{$HAS_PRIVILEGE,support_operator}}
				...
				<li>
					<form title="{!ASSIGN_TO}" action="{$PAGE_LINK*,_SEARCH:tickets:assign:ticket_id={ID}}" method="post" class="inline vertical-alignment" autocomplete="off">
						...
						<input {+START,IF,{$MOBILE}} autocorrect="off"{+END} autocomplete="off" maxlength="255" class="form-control form-control-inline input-username js-focus-update-ajax-member-list js-keyup-update-ajax-member-list" type="text" id="assign-username" name="username" value="{$USERNAME*}" />
