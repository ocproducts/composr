...
<form title="{!PRIMARY_PAGE_FORM}" method="get" action="{$URL_FOR_GET_FORM*,{$SELF_URL,0,1}}" autocomplete="off">
	...
	<div>
		<p>
			<label for="type_code"><span class="field-name">{!PRODUCT}:</span> <select id="type_code" name="type_code" class="form-control form-control-inline">{PRODUCTS}</select></label>
			<label class="horiz-field-sep" for="purchase_id"><span class="field-name">{!PURCHASE_ID}:</span> <input maxlength="80" id="purchase_id" class="form-control form-control-inline" name="purchase_id" size="10" value="{PURCHASE_ID*}" type="text" /></label>
