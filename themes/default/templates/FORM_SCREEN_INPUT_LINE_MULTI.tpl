...
<div data-tpl="formScreenInputLineMulti" class="multi-field">
	...
	<input {+START,IF_PASSED,MAXLENGTH} maxlength="{MAXLENGTH*}"{+END} tabindex="{TABINDEX*}" class="form-control {+START,IF,{$EQ,{CLASS},email}}form-control-inline{+END} input-{$REPLACE,_,-,{CLASS*}}{REQUIRED*} js-keypress-ensure-next-field" size="{$?,{$MOBILE},34,40}" type="{$?,{$EQ,{CLASS},integer},number,text}" id="{$REPLACE,[],_,{NAME_STUB*}}{I*}" name="{NAME_STUB*}{+START,IF,{$NOT,{$IN_STR,{NAME_STUB},[]}}}{I*}{+END}" value="{DEFAULT*}"{+START,IF_PASSED,PATTERN} pattern="{PATTERN*}"{+END} />
