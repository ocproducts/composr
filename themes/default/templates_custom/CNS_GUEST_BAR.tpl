<section id="tray_{!MEMBER|}" class="box cns_information_bar_outer">
	<h2 class="toggleable_tray_title">
		<a class="toggleable_tray_button inline_desktop" href="#" onclick="return toggleable_tray(this.parentNode.parentNode,false,'{!MEMBER|}');"><img alt="{!CONTRACT}: {$STRIP_TAGS,{!_LOGIN}}" title="{!CONTRACT}" src="{$IMG*,1x/trays/contract2}" srcset="{$IMG*,2x/trays/contract2} 2x" /></a>
			...
			<div class="cns_guest_column cns_guest_column_a">
				<form title="{!_LOGIN}" onsubmit="if (check_field_for_blankness(this.elements['login_username'],event)) { disable_button_just_clicked(this); return true; } return false;" action="{LOGIN_URL*}" method="post" class="inline" autocomplete="on">
					...
					<div>
						...
						<input size="15" type="text" onfocus="placeholder_focus(this);" onblur="placeholder_blur(this);" class="field_input_non_filled" alt="{!USERNAME}" value="{!USERNAME}" id="member_bar_login_username" name="login_username" />
