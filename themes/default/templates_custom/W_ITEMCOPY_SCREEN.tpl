...
<form method="post" enctype="multipart/form-data" action="{$PAGE_LINK*,_SELF:_SELF:uploading=1}" autocomplete="off">
	...
	<div class="wide-table-wrap"><table class="map-table autosized-table results-table wide-table responsive-blocked-table"><tbody>
		...
		{+START,IF_PASSED,X}{+START,IF_PASSED,Y}{+START,IF_PASSED,REALM}
			<tr>
				<th>X</th>
				<td><label class="accessibility-hidden" for="new_x">X</label><input type="text" size="20" id="new_x" name="new_x" value="{X*}" /></td>
				<td>{!W_EG_MOVE_X}</td>
			</tr>
			<tr>
				<th>Y</th>
				<td><label class="accessibility-hidden" for="new_y">Y</label><input type="text" size="20" id="new_y" name="new_y" value="{Y*}" /></td>
				<td>{!W_EG_MOVE_X}</td>
			</tr>
			<tr>
				<th>{!W_REALM}</th>
				<td><label class="accessibility-hidden" for="new_realm">{!W_REALM}</label><input type="text" size="20" id="new_realm" name="new_realm" value="{REALM*}" /></td>
				<td>{!W_EG_MOVE_X}</td>
			</tr>
		{+END}{+END}{+END}
		{+START,IF_PASSED,OWNER}
			<tr>
				<th>{!OWNER}</th>
				<td><label class="accessibility-hidden" for="new_owner">{!OWNER}</label><input type="text" size="20" id="new_owner" name="new_owner" value="{OWNER*}" /></td>
				<td>{!W_EG_OWNER}</td>
			</tr>
		{+END}
	</tbody></table></div>
