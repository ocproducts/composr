...
{+START,IF_NON_PASSED_OR_FALSE,WYSIWYG_EDITABLE}
	{+START,IF,{$AND,{FRAMED},{THUMB}}}
		{+START,IF,{$HAS_ZONE_ACCESS,adminzone}}
			<p class="associated-link associated-links-block-group"{+START,IF,{$NEQ,{WIDTH}x{HEIGHT},{$CONFIG_OPTION,thumb_width}x{$CONFIG_OPTION,thumb_width}}} style="width: {$ADD*,{WIDTH},10}px"{+END}>
