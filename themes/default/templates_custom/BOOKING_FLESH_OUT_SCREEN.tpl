...
<div data-require-javascript="booking" data-tpl="bookingFleshOutScreen">
	...
	<form action="{POST_URL*}" method="post" autocomplete="off">
		...
		<p class="proceed-button">
			<input class="button-screen buttons--proceed" type="submit" value="{$?,{$IS_GUEST},{!PROCEED},{!BOOK}}" />
		</p>
	</form>
	...
	<form action="{BACK_URL*}" method="post" autocomplete="off">
		<div>
			{HIDDEN}
			<input type="image" title="{!NEXT_ITEM_BACK}" alt="{!NEXT_ITEM_BACK}" width="48" height="48" src="{$IMG*,icons/48x48/menu/_generic_admin/back}" /></p>
		</div>
	</form>
</div>
