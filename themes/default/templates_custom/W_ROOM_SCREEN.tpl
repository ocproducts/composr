...
<form method="post" enctype="multipart/form-data" action="{$PAGE_LINK*,_SELF:_SELF:uploading=1}" autocomplete="off">
	...
	<div class="wide-table-wrap"><table class="map-table autosized-table results-table wide-table responsive-blocked-table"><tbody>
		...
		<tr>
			<th>{!W_ROOM_TEXT}</th>
			<td><label class="accessibility-hidden" for="text">{!W_ROOM_TEXT}</label><input type="text" size="20" name="text" id="text" class="form-control form-control-inline" value="{ROOM_TEXT*}" /></td>
			<td>{!W_EG_ROOM_TEXT}</td>
		</tr>
		<tr>
			<th>{!W_ENTRANCE_QUESTION}</th>
			<td><label class="accessibility-hidden" for="password_question">{!W_ENTRANCE_QUESTION}</label><input type="text" size="20" name="password_question" id="password_question" class="form-control form-control-inline" value="{PASSWORD_QUESTION*}" /></td>
			<td>{!W_EG_ENTRANCE_QUESTION}</td>
		</tr>
		<tr>
			<th>{!W_ENTRANCE_ANSWER}</th>
			<td><label class="accessibility-hidden" for="password_answer">{!W_ENTRANCE_ANSWER}</label><input type="text" size="20" name="password_answer" id="password_answer" class="form-control form-control-inline" value="{PASSWORD_ANSWER*}" /></td>
			<td>{!W_EG_ENTRANCE_ANSWER}</td>
		</tr>
		<tr>
			<th>{!W_ENTRANCE_FAIL_MESSAGE}</th>
			<td><label class="accessibility-hidden" for="password_fail_message">{!W_ENTRANCE_FAIL_MESSAGE}</label><input type="text" size="20" name="password_fail_message" id="password_fail_message" class="form-control form-control-inline" value="{PASSWORD_FAIL_MESSAGE*}" /></td>
			<td>{!W_EG_ENTRANCE_FAIL_MESSAGE}</td>
		</tr>
		<tr>
			<th>{!W_REQUIRED_ITEM}</th>
			<td><label class="accessibility-hidden" for="required_item">{!W_REQUIRED_ITEM}</label><input type="text" size="20" name="required_item" id="required_item" class="form-control form-control-inline" value="{REQUIRED_ITEM*}" /></td>
		...
		{+START,IF_PASSED,X}{+START,IF_PASSED,Y}{+START,IF_PASSED,REALM}
			<tr>
					<th>X</th>
					<td><label class="accessibility-hidden" for="new_x">X</label><input type="text" size="20" name="new_x" id="new_x" class="form-control form-control-inline" value="{X*}" /></td>
					<td>{!W_EG_MOVE_ROOM}</td>
			</tr>
			<tr>
					<th>Y</th>
					<td><label class="accessibility-hidden" for="new_y">Y</label><input type="text" size="20" name="new_y" id="new_y" class="form-control form-control-inline" value="{Y*}" /></td>
					<td>{!W_EG_MOVE_ROOM}</td>
			</tr>
			<tr>
					<th>{!W_REALM}</th>
					<td><label class="accessibility-hidden" for="new_realm">{!W_REALM}</label><input type="text" size="20" name="new_realm" id="new_realm" class="form-control form-control-inline" value="{REALM*}" /></td>
					<td>{!W_EG_MOVE_ROOM}</td>
			</tr>
		{+END}{+END}{+END}
