...
<aside class="screen-actions-outer box" data-tpl="blockMainScreenActions" data-tpl-params="{+START,PARAMS_JSON,EASY_SELF_URL}{_*}{+END}">
<nav class="screen-actions box-inner">
	...
	{+START,IF_EMPTY,{$CONFIG_OPTION,facebook_appid}}
		<div class="facebook"><a class="link-exempt js-click-add-to-facebook" target="_blank" title="{!ADD_TO_FACEBOOK} {!LINK_NEW_WINDOW}" href="http://www.facebook.com/sharer.php?u={EASY_SELF_URL.*}">{+START,INCLUDE,ICON}NAME=links/facebook{+END} <span>{!ADD_TO_FACEBOOK}</span></a></div>
	{+END}
	<div class="twitter"><a class="link-exempt js-click-add-to-twitter" target="_blank" title="{!ADD_TO_TWITTER} {!LINK_NEW_WINDOW}" href="http://twitter.com/home?status=RT%20{EASY_SELF_URL*}">{+START,INCLUDE,ICON}NAME=links/twitter{+END} <span>{!ADD_TO_TWITTER}</span></a></div>
	<div class="stumbleupon"><a class="link-exempt js-click-add-to-stumbleupon" target="_blank" title="{!ADD_TO_STUMBLEUPON} {!LINK_NEW_WINDOW}" href="http://www.stumbleupon.com/submit?url={EASY_SELF_URL*}">{+START,INCLUDE,ICON}NAME=links/stumbleupon{+END} <span>{!ADD_TO_STUMBLEUPON}</span></a></div>
	<div class="linkedin"><a class="link-exempt js-click-add-to-linkedin" target="_blank" title="{!ADD_TO_LINKEDIN} {!LINK_NEW_WINDOW}" href="https://www.linkedin.com/shareArticle?url={EASY_SELF_URL.*}&amp;title={$METADATA.*,title}&amp;mini=true">{+START,INCLUDE,ICON}NAME=links/linkedin{+END} <span>{!ADD_TO_LINKEDIN}</span></a></div>
	{+START,IF_NON_EMPTY,{$METADATA,image}}
		<div class="pinterest"><a class="link-exempt js-click-add-to-pinterest" target="_blank" title="{!ADD_TO_PINTEREST} {!LINK_NEW_WINDOW}" href="https://pinterest.com/pin/create/button/?url={EASY_SELF_URL.*}&amp;media={$METADATA.*,image}&amp;description={$METADATA.*,title}">{+START,INCLUDE,ICON}NAME=links/pinterest{+END} <span>{!ADD_TO_PINTEREST}</span></a></div>
	{+END}
	<div class="tumblr"><a class="link-exempt js-click-add-to-tumblr" target="_blank" title="{!ADD_TO_TUMBLR} {!LINK_NEW_WINDOW}" href="http://tumblr.com/widgets/share/tool?canonicalUrl={EASY_SELF_URL*}&amp;title={$METADATA.*,title}">{+START,INCLUDE,ICON}NAME=links/tumblr{+END} <span>{!ADD_TO_TUMBLR}</span></a></div>
	<div class="vk"><a class="link-exempt js-click-add-to-vk" target="_blank" title="{!ADD_TO_VK} {!LINK_NEW_WINDOW}" href="https://vk.com/share.php?url={EASY_SELF_URL.*}">{+START,INCLUDE,ICON}NAME=links/vk{+END} <span>{!ADD_TO_VK}</span></a></div>
