...
<nav class="screen-actions box-inner">
	...
	{+START,IF_EMPTY,{$CONFIG_OPTION,facebook_appid}}
		<div class="facebook"><a class="link-exempt js-click-add-to-facebook" target="_blank" title="{!ADD_TO_FACEBOOK} {!LINK_NEW_WINDOW}" href="http://www.facebook.com/sharer.php?u={URL_ENCODED_CANONICAL_URL.*}">{+START,INCLUDE,ICON}NAME=links/facebook{+END} <span>{!ADD_TO_FACEBOOK}</span></a></div>
	{+END}
	<div class="twitter"><a class="link-exempt js-click-add-to-twitter" target="_blank" title="{!ADD_TO_TWITTER} {!LINK_NEW_WINDOW}" href="http://twitter.com/home?status=RT%20{URL_ENCODED_CANONICAL_URL*}">{+START,INCLUDE,ICON}NAME=links/twitter{+END} <span>{!ADD_TO_TWITTER}</span></a></div>
	...
	{+END}
	<div class="tumblr"><a class="link-exempt js-click-add-to-tumblr" target="_blank" title="{!ADD_TO_TUMBLR} {!LINK_NEW_WINDOW}" href="http://tumblr.com/widgets/share/tool?canonicalUrl={URL_ENCODED_CANONICAL_URL*}&amp;title={$METADATA.*,title}">{+START,INCLUDE,ICON}NAME=links/tumblr{+END} <span>{!ADD_TO_TUMBLR}</span></a></div>
