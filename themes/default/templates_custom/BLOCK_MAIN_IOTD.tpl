<div class="hslice">
	<section class="box box---block-main-iotd"><div class="box-inner">
		...
		<ul class="horizontal-links associated-links-block-group force-margin">
			{+START,IF_NON_EMPTY,{VIEW_URL}}<li><a href="{VIEW_URL*}" title="{!VIEW}: {!IOTD} #{ID*}">{!VIEW}</a>{+START,IF,{$NOT,{$MATCH_KEY_MATCH,forum:topicview}}}{+START,IF_PASSED_AND_TRUE,COMMENT_COUNT} <span class="comment-count">{$COMMENT_COUNT,iotds,{ID}}</span>{+END}{+END}{+END}</li>
