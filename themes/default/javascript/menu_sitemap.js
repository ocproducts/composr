...
function generate_menu_sitemap(save_to, structure, the_level) {
    ...
    function _generate_menu_sitemap(target, node, the_level) {
        ...
        li.dataset.view = 'ToggleableTray';
        ...
        if (node.children.length != 0) {
            ...
            if (the_level < 2) {// High-levels start expanded
                ...
                expand_img.src = $cms.img("{$IMG#,1x/trays/contract}");
                expand_img.srcset = $cms.img("{$IMG#,2x/trays/contract}") + ' 2x';
            } else {
                expand_img.alt = "{!EXPAND^#}";
                expand_img.src = $cms.img("{$IMG#,1x/trays/expand}");
                expand_img.srcset = $cms.img("{$IMG#,2x/trays/expand}") + ' 2x';
