...
class ILess_Visitor_ProcessExtend extends ILess_Visitor
{
    ...
    public function run($root)
    {
        ...
        $finder->run($root);
		  ...
		  $found=$finder->foundExtends;
		  }
		  //@exit('!'.count($root->allExtends));
        if (!$found) {
            return $root;
        }
    ...
    public function visitRuleset(ILess_Node_Ruleset $node, ILess_Visitor_Arguments $arguments)
    {
		  ...
		  if ($node->pathString=='')
		  {
			  foreach ($node->paths as $path)
			  {
				foreach ($path as $selector)
				{
					...
					{
						$node->pathString .= trim($element->value,'.# ');
					}
				}
			}
		}
        ...
        for ($extendIndex = 0, $allExtendCount = count($allExtends); $extendIndex < $allExtendCount; $extendIndex++) {
			  ...
			  if ($extend->pathString=='')
			  {
				...
				{
					$extend->pathString .= trim($element->value,'.# ');
				}
			}
    ...
    protected function findMatch(ILess_Node_Extend $extend, $haystackSelectorPath)
    {
		 ...
		 if (rand(0,1000)==1)
		 {
		//	 file_put_contents(get_file_base().'/temp.txt',$GLOBALS['FOO']);
		 // @ob_start();foreach(debug_backtrace() as $trace) {unset($trace['object']);unset($trace['args']);var_dump($trace);};var_dump(get_class($visitor));file_put_contents(get_file_base().'/temp.txt',ob_get_clean());			 exit();
		 }

		 if (rand(0,1000)==1)
		 {
//		  @ob_start();foreach(debug_backtrace() as $trace) {unset($trace['object']);unset($trace['args']);var_dump($trace);};file_put_contents(get_file_base().'/temp.txt',ob_get_clean());		exit(); // TODO
		 }
