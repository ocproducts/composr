...
foreach ($group_points as $group_id => $points) {
    if (($points['p_points_one_off'] != 0) || (in_array($group_id, $my_groups))) {
        ...
        echo '<tr>
            ...
            <td>' . (in_array($group_id, $my_groups) ? '<img src="' . escape_html(find_theme_image('icons/checklist/checklist1')) . '" /> Yes' : '<img src="' . escape_html(find_theme_image('icons/checklist/checklist0')) . '" /> No') . '</td>
        </tr>';
