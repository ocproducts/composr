...
class Hook_pointstore_giftr
{
    ...
    public function action()
    {
        ...
        foreach ($rows as $gift) {
            $gift_url = build_url(array('page' => 'pointstore', 'type' => 'action_done', 'id' => 'giftr', 'gift' => $gift['id'], 'username' => $username), get_module_zone('pointstore'));
        ...
        $categories = collapse_1d_complexity('category', $GLOBALS['SITE_DB']->query_select('giftr', array('DISTINCT category'), array(), 'ORDER BY category'));
        ...
        if (array_key_exists(0, $member_rows)) {
            ...
            if (array_key_exists(0, $gift_rows)) {
                ...
                if ($anonymous == 0) {
                    ...
                    dispatch_notification('gift', null, $subject, $message, array($to_member_id), $member_id, array('use_real_from' => true));
                } else {
