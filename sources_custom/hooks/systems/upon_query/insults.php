                ...
                if ($get_reply != '') {
                    ...
                    if ($to_member == $poster_id) {
                        ...
                        if (levenshtein(trim(strtolower($post)), trim(strtolower($get_reply))) < intval(0.1 * strlen($get_reply))) {
                            ...
                            $rows = $GLOBALS['FORUM_DB']->query('SELECT g.id FROM ' . $GLOBALS['FORUM_DB']->get_table_prefix() . 'gifts g WHERE ' . $GLOBALS['SITE_DB']->translate_field_ref('reason') . ' LIKE \'' . db_encode_like('%' . $insult . '%') . '\' AND g.gift_to=' . strval($poster_id), 1, 0, true, false, array('reason' => 'SHORT_TRANS'));
