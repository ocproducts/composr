...
function assign_referral_awards($referee, $trigger)
{
    ...
    foreach ($ini_file as $ini_file_section_name => $ini_file_section) {
        if ($ini_file_section_name != 'global') {
            ...
            $one_trigger_already = ($GLOBALS['SITE_DB']->query_select_value_if_there('referees_qualified_for', 'q_referee', array('q_scheme_name' => $ini_file_section_name, 'q_referee' => $referee)) !== null);
...
function referrer_is_qualified($scheme, $member_id)
{
    ...
    if ((isset($scheme['referrer_qualified_for__event'])) && ($scheme['referrer_qualified_for__event'] == '1')) {
        if ($GLOBALS['SITE_DB']->query_value_if_there('SELECT id FROM ' . get_table_prefix() . 'calendar_events WHERE (e_submitter=' . strval($member_id) . ' OR e_member_calendar=' . strval($member_id) . ') AND validated=1') !== null) {
            $positive++;
        } else {
            $negative++;
        }
    ...
    if (addon_installed('shopping')) {
        if ((isset($scheme['referrer_qualified_for__misc_purchase'])) && ($scheme['referrer_qualified_for__misc_purchase'] == '1')) {
            if ($GLOBALS['SITE_DB']->query_value_if_there('SELECT id FROM ' . get_table_prefix() . 'shopping_orders WHERE member_id=' . strval($member_id) . ' AND ' . (db_string_equal_to('order_status', 'payment_received') . ' OR ' . db_string_equal_to('order_status', 'dispatched'))) !== null) {
                $positive++;
            } else {
                $negative++;
            }
        ...
        foreach (array_keys($scheme) as $key) {
            ...
            if (preg_match('#^referrer\_qualified\_for\_\_purchase\_(.+)$#', $key, $matches) != 0) {
                if (($GLOBALS['SITE_DB']->query_value_if_there('SELECT o.id FROM ' . get_table_prefix() . 'shopping_orders o JOIN ' . get_table_prefix() . 'shopping_order_details d ON o.id=d.p_order_id WHERE ' . db_string_equal_to('p_type_code', $matches[1]) . ' AND member_id=' . strval($member_id) . ' AND ' . (db_string_equal_to('order_status', 'payment_received') . ' OR ' . db_string_equal_to('order_status', 'dispatched'))) !== null) {
                    $positive++;
                } else {
                    $negative++;
                }
...
function referrer_report_script($ret = false)
{
    ...
    foreach ($referrals as $ref) {
        ...
        if (has_privilege(get_member(), 'member_maintenance')) {
            $data_row[do_lang('REFEREE') . ' (' . do_lang('EMAIL_ADDRESS') . ')'] = ($ref['referee_email'] === null) ? '' : $ref['referee_email'];
        }
