    ...
    if (addon_installed('shopping')) {
        ...
        foreach (array_keys($scheme) as $key) {
            ...
            if (preg_match('#^referrer\_qualified\_for\_\_purchase\_(\d+)$#', $key, $matches) != 0) {
                if (!is_null($GLOBALS['SITE_DB']->query_value_if_there('SELECT o.id FROM ' . get_table_prefix() . 'shopping_orders o JOIN ' . get_table_prefix() . 'shopping_order_details d ON o.id=d.p_order_id WHERE p_id=' . strval(intval($matches[1])) . ' AND member_id=' . strval($member_id) . ' AND ' . (db_string_equal_to('p_order_status', 'payment_received') . ' OR ' . db_string_equal_to('p_order_status', 'dispatched'))))) {
                    $positive++;
                } else {
                    $negative++;
                }
